...
**Board size is a parameter, not a literal.** `GRID_SIZE` (currently 5, was 8) drives the force table, the placement scan and the grid template in `game.tsx` — which uses an inline `gridTemplateColumns` rather than Tailwind's `grid-cols-N`, precisely so it can't fall out of sync. Test board pictures are the one place the size is implicit: `board([...])` will happily place a tile off-board, where it reads as a piece nobody can see and the first tick culls. Scenario pictures are 5 wide now, and fuzz piece counts come from `fuzzCount`, scaled to the board's area. Note some scenarios simply don't fit a small board — opposing pulls at distance 3 and 2 need six columns — which is why the crowd-vs-close board case was replaced by a column-sighting one.

**Force weights are integers and must stay integers.** `FORCE_UNIT = 705600` is divisible by every d² for d = 1..8, so every weight and every sum of weights is exact (this is what caps `GRID_SIZE` at 8). "These pulls cancel exactly" is a load-bearing statement — it's what hands control to momentum — and floats turn exact cancellation into a near-miss that silently moves the group.
